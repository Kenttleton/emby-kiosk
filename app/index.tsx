...
export default function ServerScreen() {
  ...
  const startDiscovery = () => {
    ...
    discoverServers(
      (s) => {
        if (!abortRef.current)
          setDiscovered((prev) => prev.find((x) => x.id === s.id) ? prev : [...prev, s]);
      },
      () => { setDiscovering(false); setScanDone(true); },
    );
  };
  ...
  const pickServer = async (s: EmbyServer) => {
    ...
    const connectAccessKey = record?.connectAccessKey ?? (s as any).connectAccessKey;
    const serverConnectUserId = record?.connectUserId ?? (s as any).connectUserId;
    if (
      connectAccount &&
      connectAccessKey &&
      ...
      try {
        ...
        setAuth(accessToken, { Id: localUserId, Name: connectAccount.displayName, ServerId: s.id, HasPassword: false }, 'connect');
        router.replace('/kiosk');
        return;
      } catch { /* fall through */ }
    }
    ...
    for (const u of knownUsers.slice().sort((a, b) => b.lastLoginAt.localeCompare(a.lastLoginAt))) {
      try {
        const res = await globalThis.fetch(`${s.address}/emby/System/Info`, {
          headers: { 'X-Emby-Token': u.token },
        });
        if (res.ok) {
          setAuth(u.token, { Id: u.userId, Name: u.username, ServerId: s.id, HasPassword: true }, u.loginMethod);
          router.replace('/kiosk');
          return;
        }
      } catch { /* try next */ }
  ...
  const handleManualAdd = async () => {
    ...
    try {
      ...
      if (msg.includes('Network request failed') || msg.includes('fetch'))
        setManualError('Could not reach that address. Check the IP, port, and that Emby is running.');
  ...
  const unsaved        = discovered.filter((d) => !savedServers.find((s) => s.id === d.id));

  const hasAnyServers = connectServers.length > 0 || localSaved.length > 0 || unsaved.length > 0;
  ...
  const renderServer = (item: EmbyServer, opts: { removable: boolean; badge: 'connect' | 'saved' | 'discovered' }) => {
    const badgeIcon =
      opts.badge === 'connect'    ? 'cloud-outline' :
      opts.badge === 'saved'      ? 'bookmark'      : 'radio';
    const badgeColor =
      opts.badge === 'connect'    ? Colors.accent :
      opts.badge === 'discovered' ? Colors.textMuted : Colors.textMuted;
    ...
    return (
      <Pressable style={styles.serverRow} onPress={() => pickServer(item)}>
        ...
        <Ionicons name={badgeIcon} size={14} color={badgeColor} style={{ marginRight: 4 }} />
        {opts.removable && (
          <Pressable onPress={() => removeSavedServer(item.id)} hitSlop={8} style={styles.removeBtn}>
  ...
  return (
    ...
    >
      ...
      <Pressable style={styles.connectBanner} onPress={() => router.push('/connect-login')}>
        <Ionicons name="cloud-outline" size={20} color={connectAccount ? Colors.accent : Colors.textMuted} />
        <View style={{ flex: 1 }}>
          {connectAccount ? (
            <>
              <Text style={styles.connectBannerTitle}>Emby Connect</Text>
              <Text style={styles.connectBannerSub}>{connectAccount.email}</Text>
            </>
          ) : (
            <Text style={styles.connectBannerTitle}>Sign in with Emby Connect</Text>
          )}
        </View>
        {connectAccount
      ...
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          <TextInput
            ...
            value={manualUrl}
            onChangeText={(t) => { setManualUrl(t); setManualError(null); }}
          ...
          />
          <Pressable style={[styles.btn, probing && styles.btnDisabled]} onPress={handleManualAdd} disabled={probing}>
            {probing
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.scanNote}>
          Scans your local subnet for Emby servers on ports 8096 & 8920. This may take 15–30 seconds.
        </Text>
        <Pressable
        ...
        >
          {discovering
            ...
            : <Ionicons name="wifi" size={18} color={Colors.accent} style={{ marginRight: 8 }} />
          }
        ...
        {scanDone && !discovering && discovered.length === 0 && (
          <View style={[styles.errorRow, { marginTop: 10 }]}>
            ...
            <Text style={[styles.errorText, { color: Colors.textMuted }]}>
              No Emby servers found. Check that your device is on the same network and Emby is running on port 8096 or 8920.
            </Text>
          </View>
        )}
      </View>
      ...
      {hasAnyServers && (
        <View style={[styles.card, { paddingHorizontal: 0, paddingVertical: 0, overflow: 'hidden' }]}>
          ...
          {connectServers.length > 0 && (
            <>
              <Text style={[styles.sectionLabel, styles.listSectionLabel]}>EMBY CONNECT</Text>
              <FlatList
                data={connectServers}
                keyExtractor={(s) => s.id}
                scrollEnabled={false}
                renderItem={({ item }) => renderServer(item, { removable: true, badge: 'connect' })}
          ...
          {localSaved.length > 0 && (
            <>
              {connectServers.length > 0 && <View style={styles.sectionDivider} />}
              <Text style={[styles.sectionLabel, styles.listSectionLabel]}>SAVED</Text>
              <FlatList
                data={localSaved}
                keyExtractor={(s) => s.id}
                scrollEnabled={false}
                renderItem={({ item }) => renderServer(item, { removable: true, badge: 'saved' })}
                ItemSeparatorComponent={() => <View style={styles.separator} />}
              />
            </>
          )}
          ...
          {unsaved.length > 0 && (
            <>
              {(connectServers.length > 0 || localSaved.length > 0) && <View style={styles.sectionDivider} />}
              <Text style={[styles.sectionLabel, styles.listSectionLabel]}>DISCOVERED</Text>
              <FlatList
                data={unsaved}
                keyExtractor={(s) => s.id}
                scrollEnabled={false}
                renderItem={({ item }) => renderServer(item, { removable: false, badge: 'discovered' })}
                ItemSeparatorComponent={() => <View style={styles.separator} />}
              />
            </>
          )}
...
const styles = StyleSheet.create({
  ...
  kioskLabel: { color: Colors.accent, fontSize: 11, fontWeight: '700', letterSpacing: 6, marginTop: -4 },
  ...
  listSectionLabel: { paddingHorizontal: Spacing.md, paddingTop: Spacing.sm, marginBottom: 0 },
  sectionDivider:   { height: 1, backgroundColor: Colors.divider, marginVertical: Spacing.xs },
  ...
  scanBtn: {
    ...
    borderWidth: 1.5, borderColor: Colors.accent, borderRadius: Radius.md, paddingVertical: 10,
  },
  scanBtnActive:  { backgroundColor: Colors.accentDim },
  scanBtnText:    { color: Colors.accent, fontWeight: '600', fontSize: 15 },

  serverRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: Spacing.sm, paddingHorizontal: Spacing.md },
  serverIcon: {
